...
domain.
    ...
    v      = 0                                                                  # Poisson's ratio   
    mCst   = np.array([E, v])                                                   # material constants
    g      = 10                                                                 # gravity
    ...
    for node in range (1,nodes+1):                                              # loop over nodes
        if coord[node-1,0] == 0 or coord[node-1,0]==lx:                         # roller sides (need node-1 because of zero indexing)
            bc[((node*2)-1), :] = np.array([(node*2)-1,0])           
        if coord[node-1,1] == 0:                                                # roller base
            bc[(node*2), :] = np.array([node*2,0])  
    bc = bc[~np.all(bc == 0, axis=1)].astype(int)                               # remove empty parts of bc matrix
      
    # Define data class 'mesh' with attributes to store array data in
    class mesh:
    ...
    # storing relevant data structures in mesh class 
    # (eInA added later in program where 'int' is)
    ...
    class mpDataPoint:
        def __init__(self,mpType,cmType,mpC,vp,vp0,mpM,
                     nIN,eIN,nSMe,Svp,dSvp,Fn,F,sig,
            ...
            self.nSMe = nSMe                                                    # number of stiffness entries associated with the material point
    ...
    for mp in range(0,nmp):      
        mp_point = mpDataPoint(mpType,               # mpType
                               ...
                               np.zeros([nD,1]),     # fp
                               np.zeros([nD,1]),     # u
                               np.zeros([1,nD]),     # lp  - material point domain lengths (MPM). Redefined for GIMP below
